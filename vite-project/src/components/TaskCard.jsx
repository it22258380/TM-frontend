...
function TaskCard({ task, onEdit, onDelete }) {
  ...
  return (
    <div className="bg-white shadow-lg rounded-2xl p-5 flex flex-col gap-4 border border-gray-100 hover:shadow-xl transition duration-300">
  {/* Header */}
  <div className="flex justify-between items-center">
    <h3 className="text-lg font-semibold text-gray-900 line-clamp-1">{task.title}</h3>
    <span
      className={`px-3 py-1 rounded-full text-xs font-semibold tracking-wide ${priorityColors[task.priority]}`}
    >
      {task.priority}
    </span>
  </div>
